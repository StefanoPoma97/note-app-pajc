...
public class NoteArchive extends Archive<Note> {
	...
	/**
	 * dato titolo e autore restituisce true se la nota e' segnata
	 * 
	 * @param titolo
	 * @param utente
	...
	public Boolean isPinned(String titolo, User utente) {
		...
		return notes.stream().filter(x -> x.getTitle().equals(titolo)).collect(Collectors.toList()).get(0).getPin();

	}

	/**
	 * dato titolo e autore restituisce true se la nota e' pubblica
	 * 
	 * @param titolo
	 * @param utente
	...
	public Boolean isPublic(String titolo, User utente) {
		ArrayList<Note> notes = (ArrayList<Note>) getWhere(x -> x.getAuthor().equals(utente));
		return notes.stream().filter(x -> x.getTitle().equals(titolo)).collect(Collectors.toList()).get(0).isPublic();
	...
	public Note getNoteByTitle(String n, User u) {
		...
		ArrayList<Note> out2 = (ArrayList<Note>) out.stream().filter(x -> x.getTitle().equals(n))
	...
	public ArrayList<String> getLabelsByNote(String title, User us) {
		...
		ArrayList<Note> out2 = (ArrayList<Note>) out.stream().filter(x -> x.getTitle().equals(title))
				.collect(Collectors.toList());
	...
	public ArrayList<Note> getNotesByLabel(String label, User us) {
		ArrayList<Note> out = (ArrayList<Note>) getWhere(x -> x.getAuthor().equals(us));
		ArrayList<Note> out2 = (ArrayList<Note>) out.stream().filter(x -> x.getLabel().contains(label))
				.collect(Collectors.toList());
		return out2;
	...
	public Set<User> getSharredUser(String titolo, User u) {
		ArrayList<Note> out = (ArrayList<Note>) getWhere(x -> x.getAuthor().equals(u));
		return out.stream().filter(x -> x.getTitle().equals(titolo)).collect(Collectors.toList()).get(0)
				.getSharedWith();
	}
