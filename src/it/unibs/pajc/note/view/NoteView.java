...
public class NoteView extends JPanel {
	...
	private void save(MainView view) {
		...
		ArrayList<String> titoli = (ArrayList<String>) view.getMyNote().stream().map(x -> x.getTitle())
		...
		// System.out.println("DEVO AGGIUNGERE QUESTE temporany labels:
		// "+temporanyLabels);
		note.addLabels(temporanyLabels);
		// System.out.println("HO AGGIUNTO ALLA NOTA LE TEMPORANY LABEL:
		// "+temporanyLabels);
	...
	private void refreshNoteList(MainView view) {
		...
		// metodi per far si che ogni volta sia possibile fare un refresh completo del
		// pannello
		contentList.removeAll();
		contentList.revalidate();
		view.iRefresh();
		// dimensione variabile a seconda del numero di note, e' necessario affinche la
		// ScrollBar funzioni correttamente
		...
		for (int i = 0; i < notes.size(); i++) {
			...
			int count = 0;
			// per ragioni di semplicit� il titolo non pu� contenere pi� di 20 caratteri
	...
	/**
	 * metodo per aggiornare il pannello relativo agli utenti condivisi associati
	 * alla nota che stiamo modificando
	 * 
	 * @param view
	 * @author Stefano Poma
	 */
			...
			btnNewButton.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					...
					int dialogResult = JOptionPane.showConfirmDialog(null,
							"Vuoi eliminare il seguente utente condiviso: " + btnNewButton.getActionCommand(), "Avviso",
							dialogButton);
					if (dialogResult == 0) {
						for (User u : cp) {
	...
	/**
	 * metodo per aggiornare il pannello relativo alle label associate alla nota che
	 * stiamo modificando
	 * 
	 * @@param view
	 * @author Stefano Poma
		...
		for (int i = 0; i < actualLabels.size(); i++) {
			...
			btnNewButton.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					int dialogButton = JOptionPane.YES_NO_OPTION;
					int dialogResult = JOptionPane.showConfirmDialog(null,
							"Vuoi eliminare la seguente label: " + btnNewButton.getActionCommand(), "Avviso",
							dialogButton);
					if (dialogResult == 0) {
		...
		btnAddLabel.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if (textFieldNewLabel.getText().isEmpty()
						&& comboLabelsAdd.getSelectedItem().toString().equals("Labels")) {
	...
	private void refreshButtonModify(MainView view) {
		...
		btnShare.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				ArrayList<String> cp = (ArrayList<String>) listUser.stream().map(x -> x.getName())
						.collect(Collectors.toList());
				// System.out.println("LISTA "+cp);

				String[] users = cp.toArray(new String[0]);
				// System.out.println("TUTTI GLI USER "+users[1]);
				String input = (String) JOptionPane.showInputDialog(null, "Share with...", "Choise one account",
		...
		btnDelete.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if (modifyID != null) {
					...
					showErrorMessage("Non puoi eliminare una nota che non � stata salvata");
				}
